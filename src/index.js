export class StudioLed {
	constructor({
		...
		const formatValue = (val) => {
			...
			for (let i = 0; i < diff; i++) strVal = "0" + strVal;
		...
		Object.defineProperties(self, {
			render: {
				value: () => {
					try {
						const newWidth = _config.value.length * _config.width;
						canvas.width = newWidth;
						ctx.clearRect(0, 0, newWidth, _config.height);

						for (let i = 0; i < _config.value.length; i++) {
							ctx.save();
							ctx.translate(i * _config.width, 0);
							ctx.drawImage(
								_sprites[_config.status],
								_config.value[i] * 85,
								0,
								85,
								135,
						...
						throw new Error("Error rendering LED display.");
					}
				},
				enumerable: true,
			},
			...
			addBreakpoint: {
				value: (value, width, height) => {
					...
					const oldWidth = _config.width;
					const oldHeight = _config.height;
					const updateSize = (media) => {
						/* istanbul ignore next */
						if (media.matches) {
							_config.width = width;
